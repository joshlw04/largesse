...
class Home extends Component {
  ...
  componentWillMount() { // get the state set HERE with the current user id, before anything renders...
...
class Home extends Component {
  ...
  getUser() {
    const baseURL = 'http://localhost:3000/api/v1/users/';
    request.get(`${baseURL}${firebase.auth().currentUser.uid}`).then((response) => {
      const { first_name } = response.body;
      this.setState({
        first_name: first_name,
        firebase_uid: firebase.auth().currentUser.uid,
